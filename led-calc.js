(function(){
  ...
  var CSS = '#lwcalc{--ink:#16161d;--muted:#6f6e66;--line:#ddd8cb;--accent:#8fe000;--card:#fbfaf6;font-family:Inter,system-ui,-apple-system,"Segoe UI",Roboto,Helvetica,Arial,sans-serif;color:var(--ink);max-width:1000px;margin:0 auto;line-height:1.45}'
    ...
    + '#lwcalc .lwc-seg button{flex:1;min-width:54px;padding:9px 6px;border:1px solid var(--line);background:#fff;border-radius:10px;font-size:13px;cursor:pointer;font-weight:600;color:var(--ink)}'
  ...
  var M = '<div class="lwc-grid"><div class="lwc-panel"><p class="lwc-h">Your screen</p>'
    ...
    + '<div class="lwc-field"><div class="lwc-label">Indoor or outdoor</div><div class="lwc-seg" id="lwc-env"><button data-env="indoor" aria-pressed="true">Indoor</button><button data-env="outdoor" aria-pressed="false">Outdoor</button></div></div>'
    + '<div class="lwc-field"><div class="lwc-label">Closest viewer <b><span id="lwc-dv">25</span> ft</b></div><input type="range" id="lwc-d" min="5" max="100" step="1" value="25"></div>'
    + '<div class="lwc-field"><div class="lwc-label">Pixel pitch</div><div class="lwc-seg" id="lwc-pitch"><button data-p="auto" aria-pressed="true">Auto</button><button data-p="1.5">P1.5</button><button data-p="1.9">P1.9</button><button data-p="2.6">P2.6</button><button data-p="3.9">P3.9</button></div></div>'
    ...
    + '<div class="lwc-row"><span class="k">Recommended pitch</span><span class="v" id="lwc-r-pitch">P2.6</span></div>'
    + '<div class="lwc-row"><span class="k">Panels (500mm)</span><span class="v" id="lwc-r-panels">10 &times; 5</span></div>'
    + '<div class="lwc-row"><span class="k">Built size</span><span class="v" id="lwc-r-size">16.4 &times; 8.2 ft</span></div>'
    + '<div class="lwc-row"><span class="k">Resolution</span><span class="v" id="lwc-r-res">1,920 &times; 960 px</span></div>'
    + '<div class="lwc-row"><span class="k">Brightness</span><span class="v" id="lwc-r-nits">~1,000 nits</span></div>'
    + '<div class="lwc-cost"><div class="lab">Estimated all-in day rate</div><div class="big" id="lwc-r-day">$8,000 &ndash; $11,200</div><div class="sub" id="lwc-r-total">Run total (<span class="accent">1 day</span>): same as day rate</div></div>'
  ...
  var PANEL_FT=1.6404, PXMAP={"1.5":320,"1.9":256,"2.6":192,"3.9":128};
  var state={w:16,h:9,env:"indoor",dist:25,pitch:"auto",days:1};
  function money(n){return "$"+(Math.round(n/100)*100).toLocaleString()}
  function recPitch(d){var p=d/10; if(p<=1.7)return"1.5"; if(p<=2.25)return"1.9"; if(p<=3.2)return"2.6"; return"3.9";}
  function calc(){
    var pitch = state.pitch==="auto" ? recPitch(state.dist) : state.pitch;
    var pw=Math.max(1,Math.round(state.w/PANEL_FT)), ph=Math.max(1,Math.round(state.h/PANEL_FT));
    var aw=pw*PANEL_FT, ah=ph*PANEL_FT, sqft=aw*ah, pxp=PXMAP[pitch], resW=pw*pxp, resH=ph*pxp;
    var pitchMult={"3.9":1.0,"2.6":1.2,"1.9":1.45,"1.5":1.7}[pitch];
    var envMult=state.env==="outdoor"?1.35:1.0;
    var dayRate=Math.max(3500, sqft*58*pitchMult*envMult), lo=dayRate*0.85, hi=dayRate*1.2;
    var mult=1+0.6*(state.days-1), tlo=lo*mult, thi=hi*mult;
    $("lwc-r-pitch").textContent="P"+pitch+(state.pitch==="auto"?" (auto)":"");
    ...
    $("lwc-r-nits").textContent=state.env==="outdoor"?"up to ~5,000 nits":"~800–1,500 nits";
  ...
  function seg(group,key,attr){ $(group).addEventListener("click",function(e){var b=e.target.closest("button");if(!b)return;[].forEach.call(this.children,function(c){c.setAttribute("aria-pressed","false")});b.setAttribute("aria-pressed","true");state[key]=b.getAttribute(attr);calc();}); }
  ...
  $("lwc-d").addEventListener("input",function(){state.dist=+this.value;$("lwc-dv").textContent=this.value;calc()});
  $("lwc-days").addEventListener("input",function(){state.days=+this.value;$("lwc-dayv").textContent=this.value;calc()});
  seg("lwc-env","env","data-env"); seg("lwc-pitch","pitch","data-p");
